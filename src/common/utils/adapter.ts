  ...
  return {
    ...
    certifications:
      profileResponse.certifications?.map((cert) => ({
        ...
        companyLogoUrl: cert.company?.logo?.['com.linkedin.common.VectorImage']
          ?.artifacts?.[0]
          ? `${cert.company.logo['com.linkedin.common.VectorImage'].rootUrl}${cert.company.logo['com.linkedin.common.VectorImage'].artifacts[0].fileIdentifyingUrlPathSegment}`
          : undefined,
      })) || [],
    ...
    volunteer:
      profileResponse.volunteer?.map((vol) => ({
        ...
        companyLogoUrl: vol.company?.miniCompany?.logo?.[
          ...
          ? `${vol.company.miniCompany.logo['com.linkedin.common.VectorImage'].rootUrl}${vol.company.miniCompany.logo['com.linkedin.common.VectorImage'].artifacts[0].fileIdentifyingUrlPathSegment}`
          : undefined,
      })) || [],
    ...
    honors:
      profileResponse.honors?.map((honor) => ({
        title: honor.title,
        issuer: honor.issuer,
        issueDate: new Date(honor.issueDate.year, honor.issueDate.month - 1),
      })) || [],
